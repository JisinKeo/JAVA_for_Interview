...
불변성, 안정성 등을 목적으로 다양한 요소에 적용됩니다. <br/>
...
스프링 프레임워크는 다음과 같이 다양한 빈 스코프를 지원합니다:
Singleton: 이것은 기본 스코프입니다. 스프링 IoC 컨테이너 당 하나의 인스턴스만 생성하고, 이 인스턴스를 모든 빈 요청에 대해 반환합니다.
Prototype: 이 스코프는 빈이 요청될 때마다 새로운 인스턴스를 생성하고 반환합니다.
Request: 웹 기반 스프링 애플리케이션에서 사용됩니다. HTTP 요청 당 하나의 빈 인스턴스를 생성합니다.
Session: 웹 기반 스프링 애플리케이션에서 사용됩니다. HTTP 세션당 하나의 빈 인스턴스를 생성합니다.
Application: 웹 기반 스프링 애플리케이션에서 사용됩니다. ServletContext 당 하나의 빈 인스턴스를 생성합니다. 이 스코프는 전체 웹 애플리케이션에 걸쳐 공유됩니다.
WebSocket: 웹 기반 스프링 애플리케이션에서 사용됩니다. 각 WebSocket 세션당 하나의 빈 인스턴스를 생성합니다.
각 스코프는 특정 컨텍스트에서 빈의 생성과 관리를 어떻게 할지 정의합니다. 그리고 이러한 스코프는 스프링 구성 파일이나 애노테이션을 통해 설정할 수 있습니다.

-----------------------
### 스프링 트랜잭션을 구현하기 위한 다양한 방법 중 하나를 말해주세요.
선언적 트랜잭션 관리(Declarative Transaction Management): 이 방법은 주로 XML 또는 어노테이션을 사용하여 트랜잭션을 관리합니다. 선언적 트랜잭션 관리는 AOP(Aspect Oriented Programming)를 기반으로 합니다. 어노테이션 방식은 @Transactional 어노테이션을 사용하여 비즈니스 로직과 트랜잭션 처리를 분리하며, 이 방법이 가장 일반적으로 사용되고 있습니다. XML 방식은 비즈니스 메서드와 트랜잭션 설정을 XML 파일에 명시하는 방법입니다.
